...
export default function Dashboard({ onNavigate }: { onNavigate: (tab: any) => void }) {
  ...
  // State Management untuk Data Kosong
...
function ModuleCard({ icon, title, description, color, onClick }: any) {
  ...
  return (
    ...
    >
      ...
      <div className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-full", isEmpty ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400" : "text-stone-300 dark:text-neutral-600")}>
        {isEmpty ? <Plus size={18} /> : <ArrowRight size={20} />}
      </div>
    </motion.button>
  );
}
